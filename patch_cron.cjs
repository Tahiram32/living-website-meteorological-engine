...
// Step 1: Remove the entire `if (serviceRole === "unified") { ... }` block inside `app.listen`
code = code.replace(/if \(serviceRole === "unified"\) \{\s*\/\/ Weekly Value Receipt[\s\S]*?setInterval\([\s\S]*?setTimeout\([\s\S]*?30000\);\s*\}\s*/g, '');

// Step 2: Inject the new endpoint just above `app.post("/api/pipeline/sync-weather"`
...
code = code.replace(/\/\/ 3\.6\. Autonomous Weather Routing/, weeklyReceiptEndpoint + "\n// 3.6. Autonomous Weather Routing");
