...
import { renderClientSite } from './src/renderSite';
    ...
    try {
      ...
      try {
        ...
        await db.collection("clients").doc(sourceDomain).update({
          weeklyTrades: FieldValue.increment(1),
          weeklyReferralFees: FieldValue.increment(50)
        });
    ...
    `;
      ...
      try {
        ...
        await db.collection("clients").doc(domain.toLowerCase().trim()).update({
          weeklyCalls: FieldValue.increment(1),
          weeklyRevenue: FieldValue.increment(150)
        });
...
app.post("/api/cron/weekly-value-receipt", async (req, res) => {
  try {
    ...
    for (const doc of clientsSnapshot.docs) {
      ...
      const dateOptions: Intl.DateTimeFormatOptions = { timeZone: clientTimeZone, month: 'short', day: 'numeric' };
